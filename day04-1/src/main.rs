...
fn get_answer(filename: &str) -> usize {
    ...
    let mut max_asleep_no: usize = 0;
    let mut max_asleep_min: usize = 0;

    for (guard_no, guard_asleep) in guards_asleep {
        let mut total_asleep: usize = 0;
        let mut guard_max_min: usize = 0;
        let mut guard_max_count: usize = 0;

        for (min, time_asleep) in guard_asleep {
            total_asleep += time_asleep;
            if guard_max_count < time_asleep {
                guard_max_min = min;
                guard_max_count = time_asleep;
            }
        }
        if total_asleep > max_asleep_time {
            max_asleep_time = total_asleep;
            max_asleep_no = guard_no;
            max_asleep_min = guard_max_min;
        }

    }

    max_asleep_no * max_asleep_min
